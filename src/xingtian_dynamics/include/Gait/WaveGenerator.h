...
public:
    ...
    ~WaveGenerator();
    void calcContactPhase(Vec4<T> &phaseResult, Vec4<T> &contactResult, WaveStatus status);
    T getTstance();
    T getTswing();
    T getT();
private:
    void calcWave(Vec4<T> &phase, Vec4<T> &contact, WaveStatus status);
    ...
    Vec4<T> _contact, _contactPast;
    Vec4<T> _switchStatus;          // 1: switching, 0: do not switch
