...
#ifndef FB4E9426_5BB0_44DA_BDB0_0A6C2BBD4AB9
#define FB4E9426_5BB0_44DA_BDB0_0A6C2BBD4AB9
...
#include "common/MathUtilities.h"
#include "cppTypes.h"
 ...
 */
template <typename T>
Mat3<typename T::Scalar> _rpyToRotMat(const Eigen::MatrixBase<T>& v) {
  static_assert(T::ColsAtCompileTime == 1 && T::RowsAtCompileTime == 3,
                "must have 3x1 vector");
...
Quat<typename T::Scalar> rpyToQuat(const Eigen::MatrixBase<T>& rpy) {
  static_assert(T::ColsAtCompileTime == 1 && T::RowsAtCompileTime == 3,
                "Must have 3x1 vec");
  Mat3<typename T::Scalar> R = rpyToRotMat(rpy[0],rpy[1],rpy[2]);
 ...
 */
template <typename T>             //姿态误差计算，用于计算两个四元数的乘积，scalar是类型的别名，表示是一个数字
Quat<typename T::Scalar> quatProduct(const Eigen::MatrixBase<T>& q1,
                                     const Eigen::MatrixBase<T>& q2) {
  typename T::Scalar r1 = q1[0];    //w
  typename T::Scalar r2 = q2[0];    //w
  Vec3<typename T::Scalar> v1(q1[1], q1[2], q1[3]);
  Vec3<typename T::Scalar> v2(q2[1], q2[2], q2[3]);

  typename T::Scalar r = r1 * r2 - v1.dot(v2);          //w  乘积之后的四元数  .dot 指的是点积
  Vec3<typename T::Scalar> v = r1 * v2 + r2 * v1 + v1.cross(v2);   //。cross指的是叉乘
...
#endif /* FB4E9426_5BB0_44DA_BDB0_0A6C2BBD4AB9 */
