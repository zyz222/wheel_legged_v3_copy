...
#ifndef A851F99F_DE1E_41FA_927A_A8174401E85B
#define A851F99F_DE1E_41FA_927A_A8174401E85B
 ...
 * Create spatial coordinate transformation from rotation and translation
  quat  position  6*6状态变换矩阵 R是表示两个坐标系之间的变换关系，表示坐标系间的平移偏移
  X=[ R       0
    −R×[r]×   R]
 */
template <typename T, typename T2>   //只能表示容器或者矩阵类型
auto createSXform(const Eigen::MatrixBase<T>& R,      //用于接收任意Eigen库的函数，是稠密矩阵的表达式基类，提供通用接口而不存储数据
  ...
  Mat6<typename T::Scalar> X = Mat6<typename T::Scalar>::Zero();     //T：：scalar 表示从模板参数T中提取元素类型，表明T是个非静态成员，可以动态适应不同的类型
  X.template topLeftCorner<3, 3>() = R;
  X.template bottomRightCorner<3, 3>() = R;
...
/*!
 * Compute joint transformation   X = [R       0
                                        0      R]
 */
template <typename T>
Mat6<T> jointXform(JointType joint, CoordinateAxis axis, T q) {
  ...
  if (joint == JointType::Revolute) {
    X = spatialRotation(axis, q);             //根据轴来生成一个6*6的空间旋转矩阵
...
#endif /* A851F99F_DE1E_41FA_927A_A8174401E85B */
