 ...
 */
template <typename T>
void GaitData<T>::zero() {                     //重置步态参数
  // Stop any gait transitions
  _nextGait = _currentGait;

  // General Gait descriptors
  periodTimeNominal = 0.0;      // overall period time to scale        周期时间
  initialPhase = 0.0;           // initial phase to offset             初始相位
  switchingPhaseNominal = 0.0;  // nominal phase to switch contacts     切换相位
...
// template struct GaitData<float>;                //对模板进行实例化，支持float和double
 ...
 */
template <typename T>   //这个类是根据步态类型生成步态参数，并实时调整步态变量    构造函数，在创建时会被自动调用。构造函数必须有类名完全相同
GaitScheduler<T>::GaitScheduler(CtrlComponents<T>* _ctrlComp) {       //步态初始化
  initialize();
  userParameters = _ctrlComp;
  dt = _ctrlComp->dt;
}
 ...
 */
template <typename T>    //定义模板函数，表明T是下边这个模板的类型参数，T可以是任何类型，T表示你可以为不同的类型进行创建实例
void GaitScheduler<T>::initialize() {
  ...
  gaitData._currentGait = GaitType::STAND;         //初始步态为站立

  // Zero all gait data
  gaitData.zero();                     //步态数据清零

  // Create the gait from the nominal initial
  createGait();                //创建默认步态 站立步态
  period_time_natural = gaitData.periodTimeNominal;              //将步态的周期时间和切换相位保存为自然值
 ...
 */
template <typename T>
void GaitScheduler<T>::step() {           //主程序在这里进行步态计算  //这里是主函数一直循环的地方，也是步态调度的核心功能。

  // Modify the gait with settings 
  modifyGait();                       //根据输入参数修改步态
  ...
  for (int foot = 0; foot < 4; foot++) {
    ...
    if (gaitData.gaitEnabled(foot) == 1) {
      ...
      // Check the current contact state                 这个是按周期来判断是否接触地面的！！
 ...
 */
template <typename T>
void GaitScheduler<T>::modifyGait() {                            //根据用户参数选择修改步态
  // Choose the gaits and parameters as selected
  switch ((int)userParameters->_xingtianParameters->gait_override) {
  case 0:
  ...
  case 1:
    ...
    if (gaitData._currentGait != GaitType(userParameters->_xingtianParameters->gait_type)) {
      gaitData._nextGait = GaitType(userParameters->_xingtianParameters->gait_type);
      createGait();
    }                                          
    break;
  ...
  case 2:
    ...
    // Adjust the gait parameters          //当前的周期时间和用户输入的时间不一致
    if (fabs(gaitData.periodTimeNominal - (T)userParameters->_xingtianParameters->gait_period_time) > 0.0001 ||
        fabs(gaitData.switchingPhaseNominal - (T)userParameters->_xingtianParameters->gait_switching_phase) > 0.0001) {
      // Modify the gait with new parameters if it is overrideable  
      if (gaitData.overrideable == 1) {     //表明步态数据可以被覆盖
        gaitData.periodTimeNominal = userParameters->_xingtianParameters->gait_period_time;
        gaitData.switchingPhaseNominal = userParameters->_xingtianParameters->gait_switching_phase;
        calcAuxiliaryGaitData();                //用于计算步态相关的辅助数据！！
      }
    }
    break;
...
void GaitScheduler<T>::createGait() {
  ...
  case GaitType::STAND:
    ...
    gaitData.phaseOffset << 0.5, 0.5, 0.5, 0.5;      //修改相位偏移，也就是wavegenerator
    gaitData.phaseScale << 1.0, 1.0, 1.0, 1.0;
...
void GaitScheduler<T>::calcAuxiliaryGaitData() {
  ...
  if (gaitData.overrideable == 1) {                       //允许覆盖数据
    if (userParameters->_xingtianParameters->gait_override == 2) {
    ...
    } else if (userParameters->_xingtianParameters->gait_override == 4) {
      //gaitData.periodTimeNominal = userParameters->gait_period_time;
      //gaitData.switchingPhaseNominal = userParameters->gait_switching_phase;
    }
  }
