...
template <typename T>
Vec34<T> Estimator<T>::cheater_getFeetPos(){
    Vec34<T> feetPos;
    for(int i(0); i < 4; ++i){
        feetPos.col(i) =this-> cheater_getFootPos(i);
    }
    return feetPos;
}
//返回所有机器人足端在世界坐标系下的位置
...
template <typename T>
Vec34<T> Estimator<T>::cheater_getPosFeet2BGlobal(){
    Vec34<T> feet2BPos;
    for(int i(0); i < 4; ++i){
        feet2BPos.col(i) = this->cheater_getFootPos(i) - this->cheater_getPosition();
    }
    return feet2BPos;
}
